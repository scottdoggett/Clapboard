...
export default defineSchema({
  ...
  scoringRuns: defineTable({
    startedAt: v.number(),
    /**
     * Anonymous per-installation id, generated by the extension and stored
     * only in its own chrome.storage. It exists so one person's browsing
     * can't spend the whole deployment's allowance and lock everyone else
     * out; it identifies an install, not a person, and never leaves the
